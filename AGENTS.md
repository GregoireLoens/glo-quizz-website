...
Site de quiz multijoueur temps réel (façon Kahoot). Auth « no-KYC » : pseudo + code unique `XXXX-XXXX` généré serveur, montré une seule fois. V1 livrée en juillet 2026, enrichie depuis : jokers (un de chaque par joueur, page publique `/jokers`), classement entre les questions, profil `/me`, ordre de réponses propre à chaque joueur. L'état courant vit dans `changelog.md` (local, non versionné) et au wiki.
...
2. **Un seul worker uvicorn, toujours.** L'état des parties vit en mémoire (`server/app/game/manager.py`) — jamais de `--workers N`, pas de Redis. C'est un choix d'architecture assumé (cible : VPS perso, un seul conteneur derrière le proxy Coolify) — réaffirmé le 20/08/2026 en écartant PostgreSQL : voir la page `Decisions` du wiki.
3. **Anti-triche côté serveur** : `correct_index` ne doit JAMAIS quitter le serveur avant le message `reveal` ; le chronométrage fait foi côté serveur (`time.monotonic()`), le timer client est purement cosmétique ; toutes les validations hôte-only sont revérifiées serveur. **Chaque joueur reçoit les 4 réponses dans un ordre qui lui est propre** (`_order_for`, mémorisé par joueur et par question) : `question` et `reveal` sont donc construits par destinataire, et tout `answerIndex` venant du client est un index **de sa grille**, à retraduire avant comparaison.
...
- `server/app/` — FastAPI + SQLite (sqlite3 stdlib, pas d'ORM, schéma dans `schema.sql`, idempotent, exécuté au lifespan). Endpoints REST **sync** (`def`, threadpool) ; depuis l'asyncio du jeu, accès DB via `asyncio.to_thread`.
  - `db.py` : `init_db()` = `schema.sql` puis `_migrate()`. `CREATE TABLE IF NOT EXISTS` n'ajoute rien à une table existante → toute nouvelle colonne se greffe là, en testant `PRAGMA table_info` avant l'`ALTER TABLE`. C'est le seul mécanisme de migration du projet. `_migrate()` finit par `_purge_retired_categories()` : une catégorie sortie de `config.CATEGORIES` s'inscrit dans `db.RETIRED_CATEGORIES` et ses quiz sont purgés à chaque démarrage — c'est ce qui met la prod à niveau sans intervention (fait pour « People », 20/08/2026).
...
- `client/src/` — React 19 + Vite + TS strict + Tailwind v4 + Zustand.
  ...
  - `lib/jokers.ts` : **source unique** des jokers (libellés, effets, détails, tons) — la barre de jeu, l'infobulle et la page `/jokers` la lisent tous, un libellé ne peut pas diverger.
  - Les annonces en cours de partie (joker joué, refus) ont un **emplacement réservé** : jamais d'élément inséré dans le flux qui ferait sauter l'écran pendant la lecture (leçon du 21/08).
  - Autres routes : `/me` (profil, sous `RequireAuth`, noindex) et `/jokers` (règles, publique et indexable, ouverte en nouvel onglet depuis le salon).
...
2. Multijoueur : test à 2 contextes navigateur — script prêt dans `scripts/e2e-random.js` (register via API, token dans localStorage, 2 pages, partie complète jusqu'au podium). Lancement **sur le réseau compose** (cf. contrainte n°1 — `--network host` ne marche pas sur ce PC) : `IP=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' glo-quizz-website-client-1) && docker run --rm --network glo-quizz-website_default -e BASE=http://$IP:5173 -e NODE_PATH=/usr/src/app/node_modules -v "$PWD/scripts:/work" --entrypoint node zenika/alpine-chrome:with-puppeteer /work/e2e-random.js`.
...
5. **Un test ne peut pas écrire « réponds 0 »** : chaque joueur reçoit les réponses dans son propre ordre, et l'anti-triche interdit de connaître la bonne réponse avant le reveal. Les tests serveur lisent l'ordre du joueur dans la room (helper `_slot` de `tests/test_game_ws.py`) ; un E2E navigateur ne peut qu'observer le reveal.
6. Ne jamais lancer pytest sans être sûr que `tests/conftest.py` force `DB_PATH` (base jetable) — sinon les tests écrasent la base de dev.
7. Après les vérifs navigateur : supprimer les images Docker utilisées (`docker rmi zenika/alpine-chrome…`) et purger les comptes/parties de test de la base de dev — ne garder que ce qui sert.
