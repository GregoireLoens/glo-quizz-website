...
export const useGameStore = create<GameState>()((set, get) => ({
        ...
        select: (index) => {
                if (!get().locked && get().phase === "question")
        ...
        apply: (msg) => {
                switch (msg.type) {
                        case "ping":
                                // Le watchdog de ws.ts utilise la réception de ce message ; aucun état de partie
                                // ne doit être modifié par le heartbeat.
                                break;
                        case "joined": {
                                const s = msg.state;
                                set({
                                        ...
                                        hiddenAnswers:
                                                s.jokerState?.hidden ?? [],
                                        doubleActive:
                                                s.jokerState?.double ?? false,
                                        stealTarget:
                                                s.jokerState?.stealTarget ??
                                                null,
                                        shieldActive:
                                                s.jokerState?.shield ?? false,
                                });
                                break;
                        }
                        case "players":
                        ...
                        case "reveal": {
                                const resultById = new Map(
                                        msg.results.map((r) => [r.playerId, r]),
                                );
                                set({
                                        phase: "reveal",
                                        reveal: msg,
